...
def correct(answer,lucky_list,file_path="D://英语课文//2.docx"):
    ...
    document = Document(file_path)
    document.save(file_path)#这里的问题可能是：没有定位一个准确的段落
    p = document.add_paragraph()
    document.clear()#先清空
    for i in range(len(s_list)):
        if i not in lucky_list:
            p.add_run(s_list[i]+".")#没有挖空的直接放进去就好了，记得补句点
        elif i in correct_list:
            run = p.add_run(s_list[i]+".")
            run.font.color.rgb = RGBColor(0,255,0)#写对的句子用绿色！
        else:
            for I in range(len(s_list[i].split(" "))):
                run = p.add_run(s_list[i].split(" ")[I]+" ")#补空格
                if not I in error_dict[i]:#往字典里加列表为的就是这个
                    run.font.color.rgb = RGBColor(0,255,0)#正确单词用绿色
                else:
                    run.font.color.rgb = RGBColor(255,0,0)#错误单词用红色
